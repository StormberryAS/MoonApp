 ...
 *
 * KEPT IN SYNC, BY HAND, with `ATOMIC` in `MoonApp/cities.js` and `ATOMIC_FOLD`
 * in `MoonApp/update_cities.py`. All three must agree. The Python one produces
 ...
 *
 * This is a direct port of `globalThis.foldQuery` in `MoonApp/cities.js` and it
...
object CitySearch {
     ...
     *
     * Mirrors `onCityInput` in `MoonApp/app.js`, including its ordering, and the
     ...
     * a port of `nearestCityTimezone` in `MoonApp/app.js`. Timezones are large
